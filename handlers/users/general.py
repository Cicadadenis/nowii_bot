...
async def bot_start(message: types.Message):
    ...
    try:
        ...
        with open(path, 'rb') as f:
            photo = f.read()
        await message.answer_photo(photo=photo, caption=f"{sms}")
        await message.answer(f"<b>🔼🔼🔼 Так выглядит ваш пост 🔼🔼🔼</b>\n\n <b>Акаунтов для спама:   {x}</b>\n <b>Тайминг Установлен на {ti} сек.</b>\n\n <b>Участников для спама: {z}</b>", reply_markup=await main_menu(message.from_user.id))
    except:
        
        await message.answer(sms + f"\n\n<b>🔼🔼🔼 Так выглядит ваш пост 🔼🔼🔼</b>\n\n <b>Акаунтов для спама:   {x}</b>\n <b>Тайминг Установлен на {ti} сек.</b>\n\n <b>Участников для спама: {z}</b>", reply_markup=await main_menu(message.from_user.id))
...
async def support(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await call.message.answer(f"<b>Так выглядит ваш пост</b>\n\n <b>Акаунтов для спама:   {x}</b>\n <b>Тайминг Установлен на {ti} сек.</b>\n\n <b>Участников для спама: {z}</b>", reply_markup=await main_menu(call.message.from_user.id))
    except:
        
        await call.message.edit_text(sms + f"\n\n<b>🔼🔼🔼 Так выглядит ваш пост 🔼🔼🔼</b>\n\n <b>Акаунтов для спама:   {x}</b>\n <b>Тайминг Установлен на {ti} сек.</b>\n\n <b>Участников для спама: {z}</b>", reply_markup=await main_menu(call.message.from_user.id))
