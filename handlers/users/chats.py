...
async def rmusse(call: CallbackQuery):
    ...
    for x in range(cx):
        keyboard.add(InlineKeyboardButton(text=ss[x], callback_data=ss[x]))
    keyboard.add(InlineKeyboardButton(text="🔙Назад", callback_data="back_to_main_menu"))
    await call.message.answer('<b>Какой User Удалить ?</b>\n\n', reply_markup=keyboard)
    @dp.callback_query_handler(lambda c: c.data)
    async def delus(c:types.CallbackQuery):
        ydalus = c.data
       
        ss = open('ussers.txt', 'r').read()
        ss.replace(f"set([{x}'", "")
        sa = open('ussers.txt', 'r').read()
        print(sa)
