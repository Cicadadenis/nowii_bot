...
async def rep(call: CallbackQuery):
    ...
    while i <= xx:
        try:
            ...
            if mm <= 40:
                try:
                    ...
                    await call.message.answer(
                        f"<b>Жалоба С Акаунта {acaunt} на</b> <code>{user}</code> Отправленна!\n\n")
                    o = o + 1
                    msm = msm + 1
                    mm = mm + 1
                    time.sleep(36)
            ...
            await client.disconnect()
    await call.message.answer(
                        f"<b>Жалобы все отправленны !!", reply_markup=back_to_main_menu)


@dp.callback_query_handler(text="sms")
async def ttetxt(call: CallbackQuery, state: FSMContext):
    msg_to_edit = await call.message.answer("<b>Введи текст который будет в рассылке</b>")
    @dp.message_handler(content_types=['text'])
    async def tt(message: Message):
        sms = message.text
        with open('sms.txt', 'w') as f:
            f.write(sms)
        await message.answer("Текст Изменен", reply_markup=back_to_main_menu)
            ...
            if mm <= 40:
                try:
                    ...
                    await call.message.edit_text(
                        f"<b>Рассылка с Акаунта:</b>    <code>{acaunt}</code>\n"
        ...
        try:
            ...
            while True:
                    try:
                        ...
                        time.sleep(1)
                        await call.message.answer(f"<b>Акаунт {acaunt}</b> ✅")
                        await client.disconnect()
                        i = i + 1
                        r = r + 1
                        break
